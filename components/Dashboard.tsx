  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 animate-fadeIn">
      ...
      <div className="px-8 py-8 max-w-7xl mx-auto space-y-8">
        ...
        >
          <div>
            <h2 className="text-2xl font-extrabold text-white mb-3">Upload New Material</h2>
            <p className="text-white/60 text-sm">Drop in a PDF or DOCX to start a new study session</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
            <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-2">Total Sessions</p>
            <p className="text-3xl font-extrabold text-slate-900 dark:text-white">{sessions.length}</p>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
            <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-2">Cards Reviewed</p>
            <p className="text-3xl font-extrabold text-slate-900 dark:text-white">{totalCards}</p>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
            <p className="text-xs font-medium text-slate-400 dark:text-slate-500 mb-2">Quizzes Passed</p>
          ...
          <div className="rounded-2xl p-5" style={{ background: 'linear-gradient(135deg, #4F46E5 0%, #7C3AED 100%)' }}>
            <p className="text-xs font-medium text-white/70 mb-2">Study Streak</p>
            <p className="text-3xl font-extrabold text-white">{studyStreak} {studyStreak === 1 ? 'day' : 'days'}</p>
            <p className="text-xs font-medium text-white/80 mt-1">
              {studyStreak >= 7 ? 'On fire!' : studyStreak >= 3 ? 'Keep it up!' : studyStreak >= 1 ? 'Great start!' : 'Log in daily to build your streak'}
            </p>
          </div>
        </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-[22px] font-extrabold text-slate-900 dark:text-white tracking-tight">Study Vault</h3>
          ...
          {filteredSessions.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-16 text-center border border-dashed border-slate-200 dark:border-slate-700">
              ...
              <p className="text-lg font-extrabold text-slate-900 dark:text-white mb-1">
                {searchQuery ? 'No matching sessions' : t('dashboard.vaultEmpty')}
              </p>
              <p className="text-sm text-slate-400 dark:text-slate-500">
                {searchQuery ? 'Try a different search term' : t('dashboard.startProcessing')}
              </p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-5 flex-1 space-y-4">
                      ...
                      <p className="text-xs text-slate-400 dark:text-slate-500">Created {new Date(session.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                      {/* Progress */}
                      {session.studyPlan && session.studyPlan.steps.length > 0 && (
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between">
                            <span className="text-[11px] font-medium text-slate-400 dark:text-slate-500">Progress</span>
                      ...
                      {session.quizHistory && session.quizHistory.length > 0 && (() => {
                        ...
                        return (
                          <div className="flex items-center justify-between">
                            <span className="text-[11px] font-medium text-slate-400 dark:text-slate-500">Quiz</span>
      ...
      {showRenameModal && (
        <div className="fixed inset-0 z-1002 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xl animate-fadeIn" onClick={cancelRename}>
          <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-4xl p-8 chic-shadow border border-slate-100 dark:border-slate-700 relative animate-slideUp" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 text-center mb-6">Enter a new name for this study session</p>
            <input
              type="text"
              value={renameName}
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl max-h-[92vh] sm:max-h-[85vh] rounded-t-[3rem] sm:rounded-[3rem] shadow-[0_32px_128px_-15px_rgba(0,0,0,0.5)] border border-slate-100 dark:border-slate-800 animate-slideUp flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-8 sm:px-10 pt-8 sm:pt-10 pb-6 relative shrink-0" style={{ background: 'linear-gradient(135deg, #1E1B4B 0%, #312E81 100%)' }}>
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div>
                  <h2 className="text-2xl font-black text-white tracking-tight">Customize AI</h2>
